...
const codes = [1000, 1003, 1006, 1009, 1030, 1063, 1066, 1069, 1072, 1087, 1114, 1117, 1135, 1147, 1150, 1153, 1168, 1171, 1180, 1183, 1186, 1189, 1192, 
...
const getFutureData = function() {
  fetch(`https://api.weatherapi.com/v1/forecast.json?key=378e1c6f3cbe4f8b8e9195724221603&q=Garmisch-Partenkirchen&days=4&aqi=yes&alerts=yes`)
    ...
    .then(function (data) {
      ...
      weatherSunrise.insertAdjacentHTML("afterbegin", `<i class="fas fa-arrow-up"></i><i class="fas fa-sun"></i> ${data.forecast.forecastday[0].astro.sunrise}`)
      weatherSunset.insertAdjacentHTML("afterbegin", `<i class="fas fa-arrow-down"></i><i class="fas fa-sun"></i> ${data.forecast.forecastday[0].astro.sunset}`)
      weatherMaxTemp.insertAdjacentHTML("afterbegin", `<i class="fas fa-arrow-up"></i>${data.forecast.forecastday[0].day.maxtemp_c} °C`)
      weatherMinTemp.insertAdjacentHTML("afterbegin", `<i class="fas fa-arrow-down"></i>${data.forecast.forecastday[0].day.mintemp_c} °C`)
      ...
      const weatherHourData = function(index) {
        ...
        for (let i = 0; i < codes.length; i++) {
          if (codes[i] === data.forecast.forecastday[index].day.condition.code) {
